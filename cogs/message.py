...
class Message(commands.Cog):
    ...
    @app_commands.command(name="message", description="Sends a custom message in a specified channel.")
    async def message(self, interaction: discord.Interaction, channel: discord.TextChannel, *, message: str):
        ...
        try:
            ...
            logging.info(
                f"Custom message successfully sent in '{channel}'.")
            embed = discord.Embed(
                title="Custom Message Sent", description=f"Successfully sent message in {channel} ", color=discord.Color.green())
            await interaction.followup.send(embed=embed)

        except discord.HTTPException as e:
            if e.status == 403:  # No access to channel
                logging.error(
                    f"No access to {channel}. Error: {e}")
                embed = discord.Embed(
                    title="Error", description=f"I don't have access to {channel}!", color=discord.Color.red())
                await interaction.followup.send(embed=embed)
                ...
                embed = discord.Embed(
                    title="Error", description=f"Failed to send custom message in {channel}. Please try later.", color=discord.Color.red())
                await interaction.followup.send(embed=embed)
            else:  # Other errors
                logging.error(
                    f"Error when attempting to send custom message in {channel}. Error: {e}")
                embed = discord.Embed(
                    title="Error", description=f"Failed to send custom message in {channel}.", color=discord.Color.red())
                await interaction.followup.send(embed=embed)
